...
public class ServerMaster {
		...
		public ServerMaster (String ip,int port) {
			...
			try {
				...
				log.info("Server master corriendo en puerto: "+port);
		...
		*/
		private void requestClientData(Socket client) {
			try {
				...
				log.info("Espero recibir de: "+client.getInetAddress()+":"+client.getLocalPort());
				PrintWriter outChannel = new PrintWriter (client.getOutputStream(), true);
				outChannel.println("ready");
				while(!salir){
					if((msg = inputChannel.readLine()) != null) {
						if(msg.contains("=")) {
							...
							if(msgParced[0].contains("actualizarMasterObj")) {
								...
								log.info("Mensaje parceado"+parcedData[0]+parcedData[1]);
