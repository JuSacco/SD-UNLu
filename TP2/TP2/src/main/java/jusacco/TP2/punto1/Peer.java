...
public class Peer {
	...
	public Peer (String ip,int port,String directory) {
		...
		try {
			...
			e1.printStackTrace();
			log.error("Se cayo el servidor, reiniciando cliente.");
		}
			...
			try (BufferedReader br = new BufferedReader(new FileReader(this.CONF_PATH))) {
				...
				for(String str : primaryMaster) {
					...
					if (available(ip, port)) {
						...
						if(this.masterIp != null && ((!this.masterIp.contentEquals(ip)) && (this.masterPort != port))){
							this.backUpMasterIp = ip;
							this.backUpMasterport = port;
						}
					}
				}
			} catch (IOException e) {
